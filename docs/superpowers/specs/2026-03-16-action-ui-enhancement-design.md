...
1. **レイズプリセット:** フェーズ別のサイジングボタン（スライダー値をセット）
...
- プリセットボタンをタップ → `raiseValue` state を更新（スライダーが連動）
- 確定は RAISE ボタンで行う（プリセットは値のセットのみ）
...
【自分のターン時】
┌─────────────────────────────────────┐
│  [1/3] [1/2] [2/3] [3/4] [Pot]     │  ← canRaise時のみ表示（ポストフロップ）
│  [2.5BB] [3BB] [4BB]               │  ← canRaise時のみ表示（プリフロップ）
│  [━━━━━━━━●━━━━━━━━━━━━]           │  ← canRaise時のみ表示
