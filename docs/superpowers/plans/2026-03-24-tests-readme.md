# Tests README Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `tests/README.md` as the single authoritative reference for the test suite, replacing the outdated `docs/test-guide.md`.

**Architecture:** Write `tests/README.md` combining `docs/test-guide.md` (updated) and implementation notes from `docs/ui-integration-test-plan.md` §5, then delete `docs/test-guide.md`. The prerequisite from the reorganization spec (moving `GameLoopPotManager.integration.test.ts`) is already applied.

**Tech Stack:** Markdown

---

### Task 1: Create `tests/README.md`

**Files:**
- Create: `tests/README.md`

- [ ] **Step 1: Write `tests/README.md` with the following exact content**
...
Note: In the actual file, remove the spaces inside the triple-backtick fences (`` ` `` → ` ``` `). They are escaped in this plan to avoid breaking the plan's own markdown.
...
### Task 2: Delete `docs/test-guide.md` and commit everything

**Files:**
- Delete: `docs/test-guide.md`

- [ ] **Step 1: Delete `docs/test-guide.md`**

```bash
git rm docs/test-guide.md
```
...
Expected: all tests pass (this is a documentation-only change; no test code was touched).

- [ ] **Step 3: Commit**

```bash
git add tests/README.md
git commit -m "docs(tests): add tests/README.md, replace docs/test-guide.md

- Combines docs/test-guide.md (updated) and integration test notes from
  docs/ui-integration-test-plan.md §5 into a single co-located reference
- Adds missing test files: ActionTimerBar, PreActionBar, presetCalculator,
  preAction, gameProviderModes, useActionTimer, cross-layer integration tests
- Reflects post-reorganization directory structure (gameEngine/integration/)"
```
